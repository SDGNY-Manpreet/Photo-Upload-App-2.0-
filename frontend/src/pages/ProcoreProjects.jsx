...
import { fetchProcoreProjects, uploadToProcore } from "../api/client";
...
export default function ProcoreProjects() {
  ...
  useEffect(() => {
    fetchProcoreProjects()
      .then(setProjects)
      .catch(() => setFetchError("Could not fetch Procore projects. Check API connection."))
      .finally(() => setLoading(false));
  }, []);

  const resetForm = () => {
    setSelectedProject(""); setStatus(""); setPersonName(""); setFiles([]);
    setProgress(0); setStep(""); setResult(null);
  };

  const handleUpload = async () => {
    const project = projects.find((p) => `${p.number} - ${p.name}` === selectedProject);
    if (!project) return;

    setUploading(true); setResult(null); setProgress(0);
    setStep("Uploading to Procore & SharePoint…");

    const fd = new FormData();
    fd.append("project_id", project.id);
    fd.append("project_number", project.number);
    fd.append("status", status);
    fd.append("person_name", personName);
    files.forEach((f) => fd.append("files", f));

    try {
      const res = await uploadToProcore(fd, (pct) => setProgress(Math.min(pct, 95)));
  ...
  const projectOptions = projects.map((p) => `${p.number} - ${p.name}`);
  const canUpload = selectedProject && status && personName && files.length > 0 && !uploading;

  // Determine current step for the wizard
  let currentWizardStep = 1;
  if (selectedProject) currentWizardStep = 2;
  if (selectedProject && status && personName) currentWizardStep = 3;
  ...
  return (
    <div className="max-w-2xl mx-auto w-full animate-fade-in pb-12">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Procore Projects</h1>
        <p className="text-sm text-slate-500 mt-1.5">
          Upload and organize project images. Files are synced to Procore and backed up to SharePoint automatically.
        </p>
      </div>
        ...
        {!loading && !fetchError && (
          <>
            ...
            <div className={`transition-opacity duration-300 ${currentWizardStep < 1 ? 'opacity-40 pointer-events-none' : ''}`}>
              <FormLabel stepNumber="1">Select Procore Project</FormLabel>
              <SearchableSelect
                options={projectOptions}
                value={selectedProject}
                onChange={(v) => { setSelectedProject(v); setStatus(""); setPersonName(""); setFiles([]); setResult(null); }}
                disabled={uploading}
